    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content modal-sm" style="margin:0 auto">
            ...
            <div class="modal-body">
                <form action="../report/laporanReport.php" target="_blank" method="post">
                <label>Bulan</label>
                <select name="bulan" class="form-control">
                  <option value="">Pilih</option>
                  <?php
                    $ahay = mysqli_query($kon, "SELECT DISTINCT MONTH(tgl) as bulan FROM servicereport ORDER BY bulan ASC");
                ...
                <select name="tahun" class="form-control" required>
                  <?php
                    $ahay = mysqli_query($kon, "SELECT DISTINCT YEAR(tgl) as tahun FROM servicereport ORDER BY tahun ASC");
                    while($baris = mysqli_fetch_array($ahay)) {
...
<div id="page-wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-12">
                <h1 class="page-header"><button class="btn btn-lg" style="background-color: #01633c;"><a href="report_input.php" style="color: white; text-decoration: none">+Service Report</a></button>
                    ...
                    <div class="panel-body">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                    <?php $no=1; $query = mysqli_query($kon, "SELECT * FROM servicereport ORDER BY tgl DESC");
                                        while($data = mysqli_fetch_array($query)){ ?>
                                            <tr class="odd gradeX">
                                                    <td><?= $no++; ?></td>
                                                    <td><?= date('d/m/Y',strtotime($data['tgl'])) ?></td>
                                                    <td><?= $data['ket'] ?></td>
                                                    <td><a href="../images/<?= $data['file'] ?>" target="_blank"><?= $data['file'] ?></a></td>
                                                    <td>
                                                        <a href="report_edit.php?id=<?php echo $data['id']; ?>" class="btn btn-success btn-sm"><i class="fa fa-pencil"></i></a>
                                                        <a href="delete.php?id=<?php echo $data['id'] ?>&level=report" class="btn btn-success btn-sm"><i class="fa fa-trash"></i></a>
                                                    </td>
                                                </tr>
                                        <?php } ?>
