...
require "../kon.php";
	...
	if($bulan AND $tahun){
		$result = mysqli_query($kon, "SELECT * FROM masuk JOIN stok ON masuk.idstok = stok.idstok WHERE MONTH(tgl) = '$bulan' AND YEAR(tgl) = '$tahun' ORDER BY tgl ASC");
	}else if($tahun AND empty($bulan)){
		$result = mysqli_query($kon, "SELECT * FROM masuk JOIN stok ON masuk.idstok = stok.idstok WHERE YEAR(tgl) = '$tahun' ORDER BY tgl ASC");
	}
